...
import transfer_logic from '../../../../business/user/token/transaction_demo.js'
...
const schema_request = {
  ...
  properties: {
    toAddress: {type: 'string', maxLength: 100, minLength: 1},
    tokenId: {type: 'string', maxLength: 100, minLength: 1},
    contractAddress: {type: 'string', maxLength: 100, minLength: 1},
    txHash: {type: 'string', maxLength: 100, minLength: 1},
  },
  required: ["toAddress", "tokenId", "contractAddress", 'txHash']
}
...
module.exports.action.POST = async function (ctx) {
  ...
  if (role === constants.USER_ROLE_ENDUSER) {
    ...
    if (result) {
      ctx.body = {code: 0, message: 'demo transferring txHash received', payload: {}} // TODO: fill in the payload with result content
    } else {
