...
import logic from '../../../../business/brand/token/symbol'
...
module.exports.action.GET = async function (ctx) {
  ...
  if (role === constants.USER_ROLE_BRAND) {
    /**
     *
     * This is /brand/token/symbol routing endpoint
     * Task of the endpoint: resolve request and construct response(API protocol layer)
     * DON"T write business logic here!
     */
    ...
    if (result && result.symbolVariant) {
      ctx.body = {code: 0, message: 'all brand symbols returned', payload: {symbols: result.symbolVariant}}
    } else {
